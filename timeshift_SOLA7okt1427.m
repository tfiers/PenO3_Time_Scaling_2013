...
%   Detailed explanation goes here
    ...
    for i = 2:size(framesl,1)
        ...
        correlation = [0,index1-74]; %Correlation is the largest value of the cross-correlation function and it's index.
        framelextended=[zeros(1,74),framel];
        for j = (-74):(74)% Approximately 148 data points in a 300Hz signal period. Typically, the lowest fundamental frequency of human voices is 300Hz.
            sum = 0;% Temporary value.
            for k = 1:(length_overlap-j-1)%Represents the sum in the cross-correlation formula.
                timeshifted_signal(1, k)* framelextended(1,k+j+74)
                sum = sum + timeshifted_signal(1, k)* framelextended(1,k+j+74);
            end
        ...
        new_length_overlap = length_overlap + correlation(1,2);
        timeshifted_signal(1:2, correlation(1,2):index2) = timeshifted_signal(1:2, correlation(1,2):index2) + [framel(1:length_overlap);framer(1:length_overlap)] ; % Summing up the overlapping parts.
